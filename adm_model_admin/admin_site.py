...
class AdmAdminSite(DjangoAdminSite):
    ...
    app_index_template = "adm_model_admin/admin/app_index.html"
    login_template = f"adm_auth/bootstrap/login.html"
    logout_template = f"adm_auth/bootstrap/login.html"
    ...
    def get_edc_site_title(self, request):
        ...
        return verbose_name.replace(
            'adm',
            f"adm @ {get_current_site(request).name.title()} ",
        )
